...
class OutdatedSubState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var txt:FlxText = new FlxText(0, 0, FlxG.width,
			...
			+ "\n Press escape to change your settings before you start!"
			+ "\n Otherwise, you can just press enter!"
			+ "\n This mod is made for the \n Summertime Modding Mania."
	...
	override function update(elapsed:Float)
	{
		...
		{
			leftState = true;
			var songFormat = StringTools.replace("Foreshore", " ", "-");
			var poop:String = Highscore.formatSong(songFormat, 1);

			trace(poop);
			
			PlayState.SONG = Song.loadFromJson(poop, "Foreshore");
